...
test("Commerce 404", async ({request}) => {
    ...
    const response = await request.get(`https://api.guildwars2.com/v2/commerce?access_token=${API_KEY}`)
    await expect(response.status()).toBe(404) //expected to be 404 or 503 when requested directly
...
// region /Commerce/Exchange
test("Commerce/Exchange", async ({request}) => {
